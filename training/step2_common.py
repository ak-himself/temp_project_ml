...
REPORT_DIR = PROJECT_ROOT / "logs" / "step2"
...
def load_capture_manifest() -> list[dict[str, str]]:
    ...
    with CAPTURE_LOG_FILE.open("r", encoding="utf-8") as f:
        ...
        for row in reader:
            # Support both legacy and current Step 1 log schemas.
...
def save_confusion_matrix_png(cm: np.ndarray, class_names: list[str], out_path: Path, title: str) -> None:
...
def save_metrics(
    ...
    ensure_dirs()
    json_path = REPORT_DIR / f"{model_name}_metrics.json"
    with json_path.open("w", encoding="utf-8") as f:
        json.dump(metrics, f, indent=2)

    for split_name, split_metrics in metrics["splits"].items():
        cm = np.array(split_metrics["confusion_matrix"], dtype=np.int32)
        np.save(REPORT_DIR / f"{model_name}_{split_name}_cm.npy", cm)
        save_confusion_matrix_png(
            cm,
            class_names,
            REPORT_DIR / f"{model_name}_{split_name}_cm.png",
            title=f"{model_name.upper()} {split_name.upper()} Confusion Matrix",
        )
